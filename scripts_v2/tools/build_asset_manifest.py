...
CLASS1: list[Asset] = [
    ...
    Asset(
        ...
        "dead flags below. Required because TABLE_LEG_USD_PATH's own default is set at "
        "dexlift_ur5e_delto_tableleg_env_cfg.py:52-54 -- cited by LINE, not restated as a value, "
        "because a value in prose has a shelf life a mechanism does not (it read Decomp before "
        "2026-08-23; check that line, not this sentence, for what it reads today; F24 above reads "
        "it live at generation time for exactly this reason). DEXLIFT_LEG_DECOMP=1 -- exported by "
        "every certification launcher for dexlift certs until 2026-08-29, when bead dr-76w.18 DELETED "
        "those three exports (cert_ft.sh, cert30.sh, cert_both.sh); the historical cert JSONs "
        "still carry it, so this entry stays as the record of what those runs were launched with "
        "and recorded in every historical cert JSON's plant.dexlift_env, real verified provenance -- "
        "is a VESTIGIAL export: cert_g3z4_finetune.sh:126-134's own comment states this, and this "
        ...
        "line -- 0 hits outside this manifest file itself]. THAT COMMENT ITSELF HAS A STALE CLAUSE, "
        "worth naming as the general failure mode: it reasons '...is vestigial [and] the simulated "
        "plant is identical with or without it' from the premise 'the decomposed-leg USD is now "
        "unconditional' -- true when the comment was written, false since 2026-08-23 when the "
        "default became Sdf. The conclusion (the var is dead) survives; the premise inverted. A "
        "reader trusting the premise today would conclude the OPPOSITE of the correct reproduction "
        "instruction above. Lesson for this manifest, not just that comment: a sentence naming a "
        "CURRENT VALUE ages; a sentence naming a MECHANISM (a file:line) does not, since no "
        "assertion can guard prose. Byte-identical top-level USD to the Sdf "
    ...
    Asset(
        ...
        "(dexlift_ur5e_delto_env_cfg.py, no toggle). DEXLIFT_HULLFIX=1/2/3 WAS exported (until bead "
        "dr-76w.18 removed it on 2026-08-29) by the "
        "certification launchers (run_certify.sh, cert_ft.sh, launch_task.sh) as if it "
        ...
        "SELF_COLLISIONS_ENABLED is now a hardcoded True, same R8 scope, 0 read-site hits. SYNTHESIS: "
        "three of the four collider/self-collision controls named in the published certification "
        "recipe (COLLIDERS=hullfix/hullfix2/hullfix3 -> DEXLIFT_HULLFIX, SELFCOLL=off -> "
        "DEXLIFT_NO_SELFCOLL, and DEXLIFT_LEG_DECOMP above) are ORNAMENTAL -- set, logged, and "
        "recorded in every cert JSON, but read by nothing. RESOLVED 2026-08-29 (bead dr-76w.18): all "
        "four exports were DELETED from the shell scripts rather than wired, since a flag that "
        "does nothing reads as provenance; and the leg, which had NO assertion at all, is now "
        "printed unconditionally by the [dexlift] ASSETS banner and asserted by run_certify.sh "
        "against LEG (default SquareTableLeg200mmSdf), fail-closed. Only run_certify.sh's own post-hoc "
        "WANT_COLL/WANT_SC assertion against the actual resolved plant is what keeps that ornamental "
        "surface harmless rather than silently misleading. The dexlift plant is genuinely "
        "unconditional hullfix3, confirmed by direct read of "
