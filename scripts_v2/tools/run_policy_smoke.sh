...
# DL_A6000:/home/dom_iva/run_policy_smoke.sh -- until this copy, that DL_A6000 home directory was
# the ONLY place this script existed: no git, no version history. Vendored now because training is
# moving to a new host (a parked vast.ai 1x5090) that has never seen this file. Every absolute path
# and host assumption baked into this copy is still DL_A6000-specific -- see the accompanying
...
# track at all. See generate_reset_states_policy.py's six [verify] lines, which read these back off
# the CONSTRUCTED env_cfg rather than trusting this export block.
...
LOG="/home/dom_iva/UWLab_ur5edelto/policy_smoke_lift.log"
...
timeout -s KILL 600 /home/dom_iva/UWLab/env_uwlab/bin/python -u scripts_v2/tools/generate_reset_states_policy.py \
  --checkpoint "$CKPT" \
  --num_envs 32 --smoke_steps 200 --headless \
  > "$LOG" 2>&1
echo "EXIT_CODE=$?" >> "$LOG"
