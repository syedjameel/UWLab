...
def run_part_b(args_cli, env_module_globals: dict) -> dict:
    ...
    # -- UWLab-aro2.3 INSTRUMENTATION ONLY (additive, no existing line touched): team-lead's ask
    # for a per-state cross-tab of held(contact) against AT-RESET geometric plausibility (depth>0,
    # lateral, tilt) needs per-env raw values, not just the aggregates this script already prints.
    # depth0_mm/lateral0_mm/tilt0_deg are the SAME envs' AT-RESET decomposition already computed
    # above (no cross-referencing against Part A's separate sample needed -- these are literally
    # the states Part B just replayed), and checkpoint_records[t]["held_contact"] is the SAME
    # per-env boolean already averaged into held(contact) at each printed checkpoint. Dumped
    # verbatim, not recomputed, so the cross-tab cannot silently disagree with the numbers already
    # reported.
    if getattr(args_cli, "aro2_3_dump_raw", None):
        _raw = {
            "bank_path": os.path.realpath(args_cli.bank_path),
            "n": n,
            "depth0_mm": depth0_mm.detach().cpu().tolist(),
            "lateral0_mm": lateral0_mm.detach().cpu().tolist(),
            "tilt0_deg": tilt0_deg.detach().cpu().tolist(),
            "died_at": died_at.detach().cpu().tolist(),
            "died_reason": died_reason,
            "checkpoints": {
                str(t): {
                    "held_contact": checkpoint_records[t]["held_contact"].detach().cpu().tolist(),
                    "depth_mm": checkpoint_records[t]["depth_mm"].detach().cpu().tolist(),
                    "lateral_mm": checkpoint_records[t]["lateral_mm"].detach().cpu().tolist(),
                    "tilt_deg": checkpoint_records[t]["tilt_deg"].detach().cpu().tolist(),
                }
                for t in checkpoints_s
            },
        }
        _raw_dir = os.path.dirname(os.path.abspath(args_cli.aro2_3_dump_raw)) or "."
        os.makedirs(_raw_dir, exist_ok=True)
        _raw_tmp = os.path.join(_raw_dir, f".tmp-{os.getpid()}-{os.path.basename(args_cli.aro2_3_dump_raw)}")
        with open(_raw_tmp, "w") as f:
            json.dump(_raw, f)
            f.flush()
            os.fsync(f.fileno())
        os.replace(_raw_tmp, args_cli.aro2_3_dump_raw)
        print(f"[aro2.3] wrote per-env raw dump: {args_cli.aro2_3_dump_raw}", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--aro2_3_dump_raw", type=str, default=None,
        help=(
            "UWLab-aro2.3 INSTRUMENTATION ONLY. If set, Part B additionally dumps per-env raw "
            "depth0_mm/lateral0_mm/tilt0_deg (AT RESET) and, per checkpoint, held_contact/depth_mm/"
            "lateral_mm/tilt_deg -- verbatim from the same tensors the aggregate report already "
            "prints, not recomputed -- to this JSON path, atomically. No effect on Part A/B's own "
            "computation, gating, or printed aggregates when omitted (default None)."
        ),
    )
