...
"""Generate the receptive open-top box sized for the REAL thin PCB (``realopenbox.usd`` +
``metadata.yaml``).

Mirrors ``build_openbox_usd.py`` but defaults to the real PCB footprint (140 x 100 mm) so the
insertion/near-goal (C4) task has a receptacle the PCB actually fits -- the deployed ``openbox`` is
sized for the 40 mm cube. Open-top tray (floor + 4 walls), origin at the outer bbox centre, meters.

    cavity_inner = pcb + 2*clearance ;  box_outer = cavity_inner + 2*wall ;  box_height = floor + depth

Run with any python that has ``pxr`` (e.g. ``./uwlab.sh -p``)::

    ./uwlab.sh -p scripts_v2/tools/build_realopenbox_usd.py
"""
...
_DEFAULT_OUT = os.path.join(
    ...
    "source/uwlab_assets/uwlab_assets/local/Props/Custom/RealOpenBox/realopenbox.usd",
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the real-PCB open-top box receptive USD asset.")
    parser.add_argument("--pcb-length", type=float, default=0.140, help="Mating PCB length along X (m).")
    parser.add_argument("--pcb-width", type=float, default=0.100, help="Mating PCB width along Y (m).")
    ...
    stage, _, mat = create_stage(args.output, root_name="RealOpenBox")
    for name, center, half in boxes:
        add_box(stage, f"/RealOpenBox/visuals/{name}", center=center, half_extents=half,
                collision=False, color=_COLOR)
        add_box(stage, f"/RealOpenBox/collisions/{name}", center=center, half_extents=half,
