...
# Linear-gripper action variants: IDENTICAL arm OSC (same UR5e arm + wrist_3_link IK body), only
# the binary gripper sub-action differs. LINEAR_GRIPPER_BINARY_ACTIONS commands BOTH jaws to the
# same target in meters (the prismatic mimic is unreliable, so we dual-drive; see actions.py).
@configclass
